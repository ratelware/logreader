...
namespace compressor {
	namespace lizard {
		...
		std::size_t lizard_compressor::compress_into_chunk(const char* source, char* destination, std::size_t uncompressed_size, std::size_t compressed_capacity) {
		...
		std::size_t lizard_compressor::decompress_chunk(chunk * c, char* destination) {
			...
			while (written_uncompressed_chars < c->written_uncompressed_chars) {
				...
				auto read_chars = Lizard_decompress_safe_continue(
					...
					chars_in_block,
					c->written_compressed_chars - read_compressed_chars
				);
