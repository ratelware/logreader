	...
	namespace lz4 {
		...
		std::size_t lz4_compressor::stream_compress(const char* source, char* destination, std::size_t uncompressed_size, std::size_t compressed_capacity) {
			...
			for (std::size_t bytes_to_compress = uncompressed_size; bytes_to_compress > 0; bytes_to_compress -= current_stream_call_size) {
				...
				written_compressed_bytes += SIZE_OF_BYTE_COUNT_IN_BLOCK;
				auto written = LZ4_compress_continue(encode_stream.get(), begin, destination + written_compressed_bytes, current_stream_call_size);
		...
		std::size_t lz4_compressor::decompress_chunk(chunk * c, char* destination) {
			...
			while (written_uncompressed_bytes < c->uncompressed_bytes_available) {
				...
				auto read_bytes = LZ4_decompress_safe_continue(
					decode_stream.get(),
		...
		std::size_t lz4_compressor::get_max_compressed_size(std::size_t byte_count) {
			return LZ4_compressBound(byte_count) + 2 * sizeof(uint16_t) * (byte_count / MAX_BYTES_PER_STREAM_CALL + 1);
		}
	}
}
